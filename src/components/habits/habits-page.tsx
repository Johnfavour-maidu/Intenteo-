...
const DURATION_OPTIONS = ["5 mins", "10 mins", "15 mins", "20 mins", "30 mins", "45 mins", "1 hour", "1.5 hours", "2 hours", "3 hours"]

const TOTAL_DURATION_OPTIONS = ["30 days", "60 days", "90 days", "365 days", "Indefinite"]
...
const SCROLL_CONTAINER_H = SCROLL_ITEM_H * SCROLL_VISIBLE
const SCROLL_PAD = (SCROLL_CONTAINER_H - SCROLL_ITEM_H) / 2
...
const ScrollPicker = memo(function ScrollPicker({
  ...
  const scrollToIdx = useCallback((idx: number, smooth: boolean) => {
    ...
    ref.current.scrollTo({ top: idx * SCROLL_ITEM_H, behavior: smooth ? "smooth" : "instant" })
  }, [])
...
const WeeklyCalendar = ({
  ...
  return (
    <div className="flex items-center justify-between gap-2 p-2 bg-white/50 dark:bg-white/5 rounded-xl border border-white/20">
      ...
      <div className="flex flex-col items-center gap-2">
        <div className="flex gap-1">
          {(["week", "month", "year"] as TrackerPeriod[]).map((p) => (
            <Button key={p} variant={period === p ? "default" : "outline"} size="sm" onClick={() => onPeriodChange(p)} className={period === p ? "bg-[#1E0E6B] text-white" : ""}>
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="border-b border-white/20">
            <th className="sticky left-0 z-10 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm p-3 text-left font-medium text-sm min-w-[160px]">Habit</th>
            <th className="p-3 text-left font-medium text-sm min-w-[80px]">Category</th>
            <th className="p-3 text-left font-medium text-sm min-w-[50px]">Color</th>
            <th className="p-3 text-left font-medium text-sm min-w-[70px]">Duration</th>
            {dates.map((date) => {
              const dateStr = formatDateISO(date)
              const isToday = dateStr === today
              return (
        ...
        <tbody>
          {habits.map((habit) => (
            <tr key={habit.id} className="border-b border-white/10 hover:bg-white/30 dark:hover:bg-white/5">
              ...
              <td className="p-3"><span className="text-xs text-muted-foreground flex items-center gap-1"><Clock className="h-3 w-3" />{habit.duration}</span></td>
              {dates.map((date) => {
                const dateStr = formatDateISO(date)
...
const HabitModal = ({
  ...
  const [totalDuration, setTotalDuration] = useState(habit?.totalDuration || "Indefinite")
