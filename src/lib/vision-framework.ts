...
export function seedDemoDataIfEmpty(): void {
  ...
  const milestones: RoadmapMilestone[] = [
    ...
    { id: "rm-7", visionId: "vs-5", title: "Reach 10,000 users on Intentéo", description: "Scale the platform to 10k active users.", timeHorizon: "5-years", targetYear: 2031, targetDate: "", progress: 10, status: "not-started", notes: "", relatedGoalIds: [], order: 0, createdAt: now, updatedAt: now },
  ]
  localStorage.setItem(ROADMAP_KEY, JSON.stringify(milestones))
  window.dispatchEvent(new Event("vision-framework-changed"))
}
